...
class CollectorTest {
  ...
  void validMap() {
    JavaFileObject javaFile = fromSource(
        ...
        "  abstract Map<String, java.time.LocalDate> map();",
        "",
        "  static class MapEntryMapper implements Supplier<Function<String, Map.Entry<String, java.time.LocalDate>>> {",
        "    public Function<String, Map.Entry<String, java.time.LocalDate>> get() { return null; }",
        "  }",
        "",
        "  static class MapEntryCollector<K, V> implements Supplier<Collector<Map.Entry<K, V>, ?, Map<K, V>>> {",
        "    public Collector<Map.Entry<K, V>, ?, Map<K, V>> get() { return null; }",
        "  }",
        "}");
    assertAbout(javaSources()).that(singletonList(javaFile))
        .processedWith(new Processor())
