...
class CollectorTest {
  ...
  void freeTypeVariableInCollectorMapped() {
    JavaFileObject javaFile = fromSource(
        ...
        "  abstract Set<Integer> integers();",
        "",
        "  static class MyMapper implements Supplier<Function<String, Set<Long>>> {",
        "    public Function<String, Set<Long>> get() { return null; }",
        "  }",
        "",
  ...
  void invalidBound() {
    ...
    assertAbout(javaSources()).that(singletonList(javaFile))
        .processedWith(new Processor())
        .failsToCompile()
        .withErrorContaining("not a java.util.stream.Collector or java.util.function.Supplier<java.util.stream.Collector>");
  }
